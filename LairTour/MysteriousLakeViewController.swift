...
class MysteriousLakeViewController: BaseViewController {
  
  let roomImgV = {
    let roomImg = UIImage(named: "mysterious_lake" )
  ...
  override func viewDidLoad() {
    ...
    let btnsSv = UIStackView(arrangedSubviews: [cursedChaliceBtn])
    ...
    cursedChaliceBtn.addTarget(self, action: #selector(goToCursedChalice(_:)), for: .touchUpInside)
  }
  
  // move to another room
  @objc func goToCursedChalice(_ sender: UIButton) {
    let room = CursedChaliceViewController()
