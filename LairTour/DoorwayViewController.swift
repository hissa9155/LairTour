...
class DoorwayViewController: UIViewController {
  ...
  let roomImgV = {
    let roomImg = UIImage(named: "front_door" )
  ...
  override func viewDidLoad() {
    ...
    let btnsSv = UIStackView(arrangedSubviews: [coatRoomBtn])
    ...
    coatRoomBtn.addTarget(self, action: #selector(goToCoatRoom(_:)), for: .touchUpInside)
    
  }
  
  // move to another room
  @objc func goToCoatRoom(_ sender: UIButton) {
    let coatRoom = CoatRoomViewController()
    navigationController?.pushViewController(coatRoom, animated: true)
  }
}
